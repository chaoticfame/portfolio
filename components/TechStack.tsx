        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {techGroups.map((group, i) => (
            <Reveal key={group.title} delay={i * 0.05}>
              <div className="flex h-full flex-col rounded-xl border border-hairline bg-panel/20 p-6 transition-colors duration-300 hover:border-hairline-strong hover:bg-panel/40">
                ...
                <ul className="mt-5 flex flex-col gap-3">
                  {group.items.map((item) => (
                    <li
                      key={item}
                      className="flex items-center gap-2.5 font-mono text-sm text-muted"
                    >
